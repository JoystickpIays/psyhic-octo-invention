...
Converted["_HubCredits"].Text = "UNOFFICIAL | SCRIPT IS FROM MY FRIEND DEVELOPER"
...
Converted["_ImageLabel"].Image = "rbxassetid://18211548563"
...
local function RVIFEE_fake_script() -- Fake Script: StarterGui.YARHM.Init
	...
	require(script.Parent.FUNCTIONS).notification("Welcome to YARHM Elite+ v1.17!")
...
local function NHAQOW_fake_script() -- Fake Script: StarterGui.YARHM.Murder Mystery 2
	...
	module["Name"] = "[Summer] Murder Mystery 2"
...
table.insert(module, {
	Type = "Button",
	Args = {"Throw knife to closest player (Wall Detect)", function(Self)
		if findMurderer() ~= localplayer then 
			fu.notification("Thank's to Brandon Jay to make it accurate. You're not a murderer.") 
			return 
		end
		...
		local destination = closestPlayerHRP.Position
		local direction = (destination - origin).unit
		local ray = Ray.new(origin, direction * (destination - origin).magnitude)
		local part = workspace:FindPartOnRayWithIgnoreList(ray, {localplayer.Character})
		
		if part and not part:IsDescendantOf(closestPlayer.Character) then
			fu.notification("Target player is not visible.")
			return
		end
...
local function HERR_fake_script() -- Fake Script: StarterGui.YARHM.Universal
	...
	table.insert(module, {
		Type = "Text",
		Args = {"Welcome to YARHM Elite+! The free script hub."}
	})
	table.insert(module, {
		Type = "Button",
	...
	table.insert(module, {
		...
		Args = {"Tiktok Account", function(Self)
			if setclipboard then setclipboard("@brx12k") end
...
local tagsFolder = Instance.new("Folder", script.Parent)
	...
	local specialUsernameColors = {
		["Nufty93jw"] = Color3.fromHex("#ffac33"),
	}
	
	local mainText = "YARHM Elite+ Developer"
	local mainFont = Enum.Font.GothamBold
	local specialText = "YARHM Elite+"
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"Hide YARHM Elite+/Developer tags", function(Self, state)
